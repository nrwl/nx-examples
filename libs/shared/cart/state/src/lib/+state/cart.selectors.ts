...
export function getItemCost(item: CartItem, productsState: { products: ProductsState }) {
  return getProduct(productsState, item.productId).price * item.quantity;
}

export function getTotalCost(state: CartState, productsState: { products: ProductsState }) {
